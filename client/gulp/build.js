...
gulp.task('styles', function () {
  return gulp.src('app/styles/main.scss')
    ...
    .pipe($.sass({errLogToConsole: true, onError: function(err) {
...
gulp.task('html', ['styles', 'scripts', 'partials', 'copy-html', 'copy-styles', 'copy-scripts'], function () {
  ...
  return gulp.src('app/*.html')
    ...
    .pipe($.useref())
    .pipe($.revReplace())
    // ouput to tmp then move to server in the 'copy' task
    .pipe(gulp.dest('.tmp'))
    .pipe($.size());
});
...
gulp.task('clean', function () {
  return gulp.src(['.tmp', '../server/public'], { read: false }).pipe($.clean());
});
